...
Revision ID: 15919a743bb3
Revises: a17440d8fa53
Create Date: 2025-12-25 01:57:17.077423
...
revision: str = '15919a743bb3'
down_revision: Union[str, Sequence[str], None] = 'a17440d8fa53'
...
def downgrade() -> None:
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True),
    sa.CheckConstraint('buyer_id <> seller_id', name=op.f('buyer_not_seller')),
    sa.CheckConstraint('communication_rating IS NULL OR communication_rating >= 1 AND communication_rating <= 5', name=op.f('comm_rating_range')),
    sa.CheckConstraint('condition_accuracy_rating IS NULL OR condition_accuracy_rating >= 1 AND condition_accuracy_rating <= 5', name=op.f('cond_rating_range')),
    sa.CheckConstraint('delivery_speed_rating IS NULL OR delivery_speed_rating >= 1 AND delivery_speed_rating <= 5', name=op.f('delivery_rating_range')),
    sa.CheckConstraint('rating >= 1 AND rating <= 5', name=op.f('rating_range')),
    sa.ForeignKeyConstraint(['announcement_id'], ['announcements.id'], name=op.f('ratings_announcement_id_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['buyer_id'], ['users.id'], name=op.f('ratings_buyer_id_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['seller_id'], ['users.id'], name=op.f('ratings_seller_id_fkey'), ondelete='CASCADE'),
